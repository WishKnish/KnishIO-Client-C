...
### **Avoiding Simplified APIs**

**❌ DON'T USE** (Simplified/Non-equivalent):
- `knishio_wallet_create_simple()` - Missing parameters
- `knishio_client_authenticate()` - Different auth model
- Configuration struct patterns without constructor functions

**✅ DO USE** (Full JavaScript Equivalency):
- `knishio_wallet_create_from_params()` - All 7 parameters
- `knishio_molecule_create()` - All 6 parameters  
- `knishio_atom_create_with_meta()` - All metadata support
- `knishio_client_set_secret()` - Proper C auth pattern
